...
/**
 * Lightweight FX cache for display only. Technical engines keep using the
 * instrument's native market price; US prices are converted to EUR only when
...
public final class CurrencyService {
    ...
    public static boolean hasRate() {
        return Double.isFinite(usdToEur) && usdToEur > 0;
    }
    ...
    public static void refreshIfNeeded() {
        ...
        synchronized (CurrencyService.class) {
            ...
            if (hasRate() && now - updatedAt < TTL_MS) return;
            try {
                // Yahoo symbol USDEUR=X returns EUR value of one USD.
                List<MarketDataService.Candle> data = MarketDataService.fetchDaily("USDEUR=X", "5d");
                if (data != null && !data.isEmpty()) {
                    double r = data.get(data.size() - 1).close;
                    if (Double.isFinite(r) && r > 0.3 && r < 2.0) {
                        usdToEur = r;
                        updatedAt = now;
                        return;
                    }
            ...
            try {
                // Fallback: EURUSD=X is USD value of one EUR, so invert it.
                List<MarketDataService.Candle> data = MarketDataService.fetchDaily("EURUSD=X", "5d");
                if (data != null && !data.isEmpty()) {
