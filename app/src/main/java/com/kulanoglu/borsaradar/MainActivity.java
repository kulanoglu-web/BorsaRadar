...
public class MainActivity extends Activity {
    ...
    private void renderStockDetail(String symbol,ShortPulseEngine.Result r,CatalystContextEngine.Result cx,List<MarketDataService.Candle> chart) {
        ...
        LinearLayout actions=new LinearLayout(this); Button prev=button("◀ Önceki",NAVY2), add=button("Portföye Ekle",GREEN), refreshDetail=button("Yenile",NAVY2), next=button("Sonraki ▶",NAVY2); actions.addView(prev,new LinearLayout.LayoutParams(0,-2,1)); actions.addView(add,new LinearLayout.LayoutParams(0,-2,1.15f)); actions.addView(refreshDetail,new LinearLayout.LayoutParams(0,-2,.8f)); actions.addView(next,new LinearLayout.LayoutParams(0,-2,1)); content.addView(actions); prev.setOnClickListener(v->analyzeStock(adjacentSymbol(symbol,-1),detailTimeframe)); add.setOnClickListener(v->portfolioDialog(null,symbol)); refreshDetail.setOnClickListener(v->analyzeStock(symbol,detailTimeframe)); next.setOnClickListener(v->analyzeStock(adjacentSymbol(symbol,1),detailTimeframe));
    }
