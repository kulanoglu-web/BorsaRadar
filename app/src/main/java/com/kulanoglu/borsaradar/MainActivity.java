...
public class MainActivity extends Activity {
    ...
    private void showDashboard() {
        ...
        LinearLayout heroText=new LinearLayout(this);heroText.setOrientation(LinearLayout.VERTICAL);heroText.addView(bold("Piyasayı tek ekrandan takip et",20,Color.WHITE));heroText.addView(txt("Radar • Portföy • Haber • Teknik",11,Color.rgb(145,175,200)));hero.addView(heroText,new LinearLayout.LayoutParams(0,dp(55),1));TextView bell=bold("●",16,Color.rgb(70,150,255));bell.setGravity(Gravity.CENTER);hero.addView(bell,new LinearLayout.LayoutParams(dp(38),dp(38)));content.addView(hero);
        TextView search=txt("⌕   Hisse, endeks veya şirket ara...",13,Color.rgb(185,205,220));search.setPadding(dp(14),dp(12),dp(14),dp(12));search.setBackgroundColor(NAVY2);search.setOnClickListener(v->singleStockDialog());content.addView(search);spacer(7);
        LinearLayout exchange=new LinearLayout(this);String[] ex={"BIST","Almanya","ABD","Tümü"};for(String e:ex){Button q=button(e,e.equals("BIST")?Color.rgb(25,105,220):NAVY2);q.setTextSize(10);exchange.addView(q,new LinearLayout.LayoutParams(0,dp(34),1));}content.addView(exchange);spacer(10);

        content.addView(bold("Stratejiler",15,Color.WHITE));LinearLayout r1=new LinearLayout(this),r2=new LinearLayout(this);
        Button fast=button("⚡  Hızlı Tarama\nAnlık fırsatlar",Color.rgb(25,105,220)),shortB=button("↗  Kısa Vade\nGünlük / saatlik",Color.rgb(40,61,105)),div=button("◆  Temettü\nDüzenli gelir",Color.rgb(18,83,76)),longB=button("◎  Uzun Vade\nGüçlü şirketler",Color.rgb(93,72,30));
        for(Button x:new Button[]{fast,shortB,div,longB}){x.setGravity(Gravity.START|Gravity.CENTER_VERTICAL);x.setTextSize(11);x.setPadding(dp(12),dp(7),dp(8),dp(7));}
        r1.addView(fast,new LinearLayout.LayoutParams(0,dp(62),1));r1.addView(shortB,new LinearLayout.LayoutParams(0,dp(62),1));r2.addView(div,new LinearLayout.LayoutParams(0,dp(62),1));r2.addView(longB,new LinearLayout.LayoutParams(0,dp(62),1));content.addView(r1);content.addView(r2);spacer(10);
        ...
        LinearLayout fh=new LinearLayout(this);fh.setGravity(Gravity.CENTER_VERTICAL);fh.addView(bold("Günün Öne Çıkanları",15,Color.WHITE),new LinearLayout.LayoutParams(0,dp(32),1));TextView all=txt("Tümünü Gör  ›",11,Color.rgb(90,165,255));all.setGravity(Gravity.END|Gravity.CENTER_VERTICAL);fh.addView(all,new LinearLayout.LayoutParams(0,dp(32),1));content.addView(fh);
        LinearLayout featured=card();List<RadarItem> top=new ArrayList<>(radarResults);top.removeIf(x->x.recommendation.contains("SAT")||x.recommendation.contains("RİSK"));top.sort((x,y)->Double.compare(y.score,x.score));
        if(top.isEmpty()){LinearLayout row=new LinearLayout(this);LinearLayout lt=new LinearLayout(this);lt.setOrientation(LinearLayout.VERTICAL);lt.addView(bold("Radar henüz taranmadı",13,Color.WHITE));lt.addView(txt("Fırsat hisseleri burada listelenecek",10,Color.rgb(155,180,200)));row.addView(lt,new LinearLayout.LayoutParams(0,dp(45),1));Button go=button("Tara",Color.rgb(25,105,220));go.setTextSize(10);row.addView(go,new LinearLayout.LayoutParams(dp(75),dp(38)));go.setOnClickListener(v->showRadar());featured.addView(row);}
        else for(int i=0;i<Math.min(3,top.size());i++){RadarItem x=top.get(i);LinearLayout row=new LinearLayout(this);TextView sy=bold(x.symbol,13,Color.WHITE),px=bold(money(x.price,x.symbol),12,Color.WHITE),sg=bold(x.recommendation,11,x.recommendation.contains("AL")?GREEN:AMBER);sg.setGravity(Gravity.END);row.addView(sy,new LinearLayout.LayoutParams(0,dp(32),1));row.addView(px,new LinearLayout.LayoutParams(0,dp(32),1));row.addView(sg,new LinearLayout.LayoutParams(0,dp(32),1));row.setOnClickListener(v->analyzeStock(x.symbol));featured.addView(row);}content.addView(featured);spacer(9);

        content.addView(bold("Piyasa Özeti",15,Color.WHITE));LinearLayout markets=new LinearLayout(this);String[] names={"BIST 100","DAX","S&P 500","NASDAQ"};for(String name:names){LinearLayout mc=card();mc.setPadding(dp(8),dp(7),dp(8),dp(7));mc.addView(bold(name,11,Color.WHITE));mc.addView(bold("—",14,Color.rgb(170,195,215)));mc.addView(txt("veri bekleniyor",9,Color.rgb(130,160,185)));markets.addView(mc,new LinearLayout.LayoutParams(0,dp(68),1));}content.addView(markets);spacer(9);
        LinearLayout news=card();LinearLayout nr=new LinearLayout(this);nr.setGravity(Gravity.CENTER_VERTICAL);TextView dot=bold("●",11,RED);nr.addView(dot,new LinearLayout.LayoutParams(dp(24),dp(32)));LinearLayout nt=new LinearLayout(this);nt.setOrientation(LinearLayout.VERTICAL);nt.addView(bold("Son Dakika",13,Color.WHITE));nt.addView(txt("KAP ve önemli piyasa gelişmeleri",10,Color.rgb(165,190,210)));nr.addView(nt,new LinearLayout.LayoutParams(0,dp(42),1));TextView ar=bold("›",20,Color.rgb(100,165,225));nr.addView(ar,new LinearLayout.LayoutParams(dp(25),dp(42)));news.addView(nr);content.addView(news);
        if(scanRunning){spacer(6);ProgressBar bar=new ProgressBar(this,null,android.R.attr.progressBarStyleHorizontal);bar.setMax(ALL_SYMBOLS.length);bar.setProgress(scanDone.get());content.addView(bar);}
    }

    private void showPortfolio() {
        shell("Portföy");
        LinearLayout total=card(); total.setBackgroundColor(NAVY2); total.addView(txt("TOPLAM PORTFÖY DEĞERİ",11,Color.rgb(160,185,205))); total.addView(bold(holdings.isEmpty()?"0,00 ₺":holdings.size()+" pozisyon",24,Color.WHITE)); LinearLayout ptabs=new LinearLayout(this); Button ptab=button("Portföy",Color.rgb(25,105,220)), itab=button("İşlemler",NAVY2);ptabs.addView(ptab,new LinearLayout.LayoutParams(0,dp(36),1));ptabs.addView(itab,new LinearLayout.LayoutParams(0,dp(36),1));total.addView(ptabs); content.addView(total); spacer(6);
        ...
        add.setOnClickListener(v->portfolioDialog(null,null)); refresh.setEnabled(!portfolioRefreshing); refresh.setText(portfolioRefreshing?"Güncelleniyor…":"Portföy Analiz"); refresh.setOnClickListener(v->refreshPortfolio()); spacer(8); long pts=getSharedPreferences(PREFS,Context.MODE_PRIVATE).getLong("portfolio_ts",0); if(pts>0)content.addView(txt("Son portföy güncellemesi: "+new java.text.SimpleDateFormat("dd.MM HH:mm",Locale.getDefault()).format(new java.util.Date(pts))+" • "+holdings.size()+" pozisyon",12,Color.GRAY));if(portfolioRefreshing)content.addView(txt("Teknik veriler yenileniyor; mevcut pozisyonlar korunuyor.",12,Color.GRAY));
        if(holdings.isEmpty()) { LinearLayout c=card(); c.setBackgroundColor(NAVY2); c.addView(bold("Portföy boş",19,Color.WHITE)); c.addView(txt("Hisse ekleyince maliyet, güncel fiyat, teknik görünüm ve haber/katalizör bağlamı burada görünür.",14,Color.rgb(175,198,216)));Button radar=button("Radardan Hisse Bul",GREEN);c.addView(radar);radar.setOnClickListener(v->showRadar()); content.addView(c); return; }
    ...
    private void showStockTechnical(String symbol,ShortPulseEngine.Result r){
        ...
        LinearLayout head=card();head.setBackgroundColor(NAVY2);head.addView(bold("Teknik Analiz",18,Color.WHITE));head.addView(txt("İndikatör Sinyalleri",12,Color.rgb(150,190,220)));content.addView(head);spacer(5);
        ...
        LinearLayout table=card();table.setBackgroundColor(NAVY2);for(int i=0;i<rows.length;i++){LinearLayout row=new LinearLayout(this);TextView n=bold(rows[i][0],12,Color.WHITE),desc=txt(rows[i][1],10,Color.rgb(160,185,205));LinearLayout l=new LinearLayout(this);l.setOrientation(LinearLayout.VERTICAL);l.addView(n);l.addView(desc);String signal=i<4?(r.score>=4?"AL":r.score<=1?"SAT":"İZLE"):(r.recommendation.contains("AL")?"AL":r.recommendation.contains("SAT")?"SAT":"İZLE");int col=signal.equals("AL")?GREEN:signal.equals("SAT")?RED:AMBER;TextView sv=bold(signal,11,col);sv.setGravity(Gravity.END|Gravity.CENTER_VERTICAL);row.addView(l,new LinearLayout.LayoutParams(0,dp(43),1));row.addView(sv,new LinearLayout.LayoutParams(dp(70),dp(43)));table.addView(row);}content.addView(table);spacer(6);
        LinearLayout overallBox=card();overallBox.setBackgroundColor(NAVY2);overallBox.addView(bold("Genel Teknik Görünüm",14,Color.WHITE));overallBox.addView(bold(r.recommendation+"   •   Alım Gücü %"+(int)r.confidence,18,overall));overallBox.addView(txt("Kısa vade: "+r.horizonText+"   •   Pulse "+fmt(r.score),11,Color.rgb(175,198,216)));content.addView(overallBox);
    }
    private void showStockRisk(String symbol,ShortPulseEngine.Result r){
        ...
        LinearLayout analysts=card();analysts.setBackgroundColor(NAVY2);analysts.addView(bold("Hedef Fiyat & Risk",18,Color.WHITE));analysts.addView(txt("Analist Hedefleri & Tahminleri",12,Color.rgb(150,190,220)));
        LinearLayout ar=new LinearLayout(this);String[] labels={"Ortalama\n—","Yüksek\n—","Düşük\n—","Analist\n—"};for(String x:labels){TextView t=bold(x,11,Color.WHITE);t.setGravity(Gravity.CENTER);ar.addView(t,new LinearLayout.LayoutParams(0,dp(55),1));}analysts.addView(ar);content.addView(analysts);spacer(6);
        LinearLayout horizons=card();horizons.setBackgroundColor(NAVY2);horizons.addView(bold("Zaman Ufku",14,Color.WHITE));String[] hs={"Kısa Vade","Orta Vade","Uzun Vade"};for(String h:hs){LinearLayout row=new LinearLayout(this);row.addView(txt(h,12,Color.rgb(175,198,216)),new LinearLayout.LayoutParams(0,dp(34),1));TextView v=bold("—",12,Color.WHITE);v.setGravity(Gravity.END|Gravity.CENTER_VERTICAL);row.addView(v,new LinearLayout.LayoutParams(0,dp(34),1));horizons.addView(row);}content.addView(horizons);spacer(6);
        ...
        LinearLayout actions=new LinearLayout(this);Button add=button("+ Portföye Ekle",GREEN),chart=button("Grafiğe Dön",Color.rgb(25,105,220));actions.addView(add,new LinearLayout.LayoutParams(0,dp(44),1));actions.addView(chart,new LinearLayout.LayoutParams(0,dp(44),1));content.addView(actions);Holding owned=findHolding(symbol);add.setOnClickListener(v->portfolioDialog(owned,symbol));chart.setOnClickListener(v->analyzeStock(symbol,detailTimeframe));
    }
    ...
    private void showRadar() {
        ...
        LinearLayout titleRow=new LinearLayout(this);titleRow.setGravity(Gravity.CENTER_VERTICAL);titleRow.addView(bold("Radar Sonuçları",19,Color.WHITE),new LinearLayout.LayoutParams(0,dp(38),1));TextView search=bold("⌕",20,Color.WHITE);search.setGravity(Gravity.END|Gravity.CENTER_VERTICAL);titleRow.addView(search,new LinearLayout.LayoutParams(dp(40),dp(38)));content.addView(titleRow);
        LinearLayout filters=new LinearLayout(this);String[] fs={"Tümü","AL Sinyali","İzle","SAT"};for(int i=0;i<fs.length;i++){Button q=button(fs[i],i==0?Color.rgb(25,105,220):NAVY2);q.setTextSize(10);filters.addView(q,new LinearLayout.LayoutParams(0,dp(34),1));}content.addView(filters);spacer(5);
        ...
        Button scan=button(scanRunning?"Taranıyor  "+scanDone.get()+"/"+ALL_SYMBOLS.length:"Taramayı Başlat",Color.rgb(25,105,220));scan.setEnabled(!scanRunning);scan.setOnClickListener(v->scanRadar());content.addView(scan,new LinearLayout.LayoutParams(-1,dp(42)));spacer(7);
    ...
    private void renderRadarList(List<RadarItem> items,int max) {
        ...
        int n=Math.min(max,items.size());for(int i=0;i<n;i++){RadarItem r=items.get(i);MarketDataService.Spot spot=MarketDataService.latestSpot(r.symbol);double shown=spot!=null&&spot.price>0?spot.price:r.price;double ch=spot!=null?spot.changePct:r.changePct;String sig=r.recommendation.contains("SAT")||r.recommendation.contains("RİSK")?"SAT":r.recommendation.contains("AL")?"AL":"İZLE";int col=sig.equals("AL")?GREEN:sig.equals("SAT")?RED:AMBER;
            LinearLayout row=card();row.setGravity(Gravity.CENTER_VERTICAL);TextView sy=bold(r.symbol,13,Color.WHITE);LinearLayout syBox=new LinearLayout(this);syBox.setOrientation(LinearLayout.VERTICAL);syBox.addView(sy);syBox.addView(txt(r.horizon,9,Color.rgb(135,165,190)));TextView px=bold(money(shown,r.symbol),12,Color.WHITE);px.setGravity(Gravity.END|Gravity.CENTER_VERTICAL);TextView change=bold(String.format(Locale.US,"%+.2f%%",ch),11,ch>=0?GREEN:RED);change.setGravity(Gravity.END|Gravity.CENTER_VERTICAL);TextView signal=bold(sig,11,col);signal.setGravity(Gravity.END|Gravity.CENTER_VERTICAL);row.addView(syBox,new LinearLayout.LayoutParams(0,dp(43),1));row.addView(px,new LinearLayout.LayoutParams(0,dp(43),1));row.addView(change,new LinearLayout.LayoutParams(0,dp(43),1));row.addView(signal,new LinearLayout.LayoutParams(0,dp(43),1));row.setOnClickListener(v->analyzeStock(r.symbol));content.addView(row);spacer(2);
        }
    }
