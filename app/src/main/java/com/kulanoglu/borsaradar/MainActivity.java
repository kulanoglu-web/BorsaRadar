...
public class MainActivity extends Activity {
    ...
    private void showRadar() {
        ...
        Button scan=button(scanRunning?"Tarama "+scanDone.get()+"/"+ALL_SYMBOLS.length:"Tüm BIST'i Tara",GREEN); if(!radarResults.isEmpty()&&!scanRunning)top.addView(txt("Mevcut sonuçlar korunur; yeni tarama yalnızca tamamlandığında listeyi yeniler.",12,Color.GRAY)); top.addView(scan); scan.setEnabled(!scanRunning); scan.setOnClickListener(v->scanRadar()); content.addView(top); spacer(8);
