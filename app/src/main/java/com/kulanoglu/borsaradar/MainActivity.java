...
public class MainActivity extends Activity {
    ...
    private void showBaskets() {
        ...
        if(!radarResults.isEmpty()){List<RadarItem> all=new ArrayList<>(radarResults);all.sort((x,y)->Double.compare(y.score,x.score));LinearLayout preview=card();preview.addView(bold("Son Radar Özeti",13,Color.WHITE));preview.addView(txt(all.size()+" hisse değerlendirildi • sonuçlar Radar ekranında",10,Color.rgb(155,185,207)));Button open=button("Radar Sonuçlarını Aç",Color.rgb(25,105,220));open.setOnClickListener(v->showRadar());preview.addView(open);content.addView(preview);}
    }
