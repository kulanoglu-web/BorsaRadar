...
public class MainActivity extends Activity {
    ...
    private void showBaskets() {
        shell("100.000 TL • 3 Sepet"); if(radarResults.isEmpty()){LinearLayout c=card();c.addView(bold(scanRunning?"Radar taraması devam ediyor":"Önce radar taraması gerekiyor",18,NAVY));if(scanRunning)c.addView(txt("Tamamlanınca 3 Sepet son geçerli radar sonucundan otomatik üretilebilir.",12,Color.GRAY));c.addView(txt("Tarama bir kez tamamlanınca sonuç kaydedilir; ekrandan çıksan da kaybolmaz.",13,Color.DKGRAY));Button go=button("Radarı Aç",GREEN);c.addView(go);go.setOnClickListener(v->showRadar());content.addView(c);return;}
