...
public class MainActivity extends Activity {
    ...
    private void renderRadarList(List<RadarItem> items,int max) {
        items.sort((a,b)->Double.compare(b.score,a.score)); content.addView(bold("En güçlü adaylar",18,NAVY));content.addView(txt("İlk "+Math.min(max,items.size())+" sonuç gösteriliyor.",11,Color.GRAY));content.addView(txt("Detaya girip Önceki/Sonraki ile bu adaylar arasında hızlı geçiş yapabilirsin.",12,Color.GRAY)); int n=Math.min(max,items.size());
