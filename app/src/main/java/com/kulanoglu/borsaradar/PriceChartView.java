...
public final class PriceChartView extends View {
    ...
    @Override public boolean onTouchEvent(MotionEvent e){
        ...
        if(e.getAction()==MotionEvent.ACTION_DOWN){float lx=e.getX(),ly=e.getY();if(ly>=dp(28)&&ly<=dp(52)){float left=getPaddingLeft()+dp(3);if(lx>=left&&lx<legendEmaRight)showEma=!showEma;else if(lx<legendBbRight)showBands=!showBands;else if(lx<legendVwapRight)showVwap=!showVwap;else if(lx<legendLevelsRight)showLevels=!showLevels;else if(lx<legendVolumeRight)showVolume=!showVolume;else return true;selectedIndex=-1;invalidate();return true;}downX=lastX=touchX=lx;panning=false;return true;}
    ...
    private void drawLegend(Canvas canvas,float left,float y){paint.setTextSize(dp(9));String[] names={"EMA","BB","VWAP","S/R","HAC"};boolean[] on={showEma,showBands,showVwap,showLevels,showVolume};float x=left;for(int i=0;i<names.length;i++){paint.setColor(on[i]?Color.WHITE:Color.rgb(105,120,138));String t=(on[i]?"● ":"○ ")+names[i];canvas.drawText(t,x,y,paint);x+=paint.measureText(t)+dp(12);if(i==0)legendEmaRight=x;else if(i==1)legendBbRight=x;else if(i==2)legendVwapRight=x;else if(i==3)legendLevelsRight=x;else legendVolumeRight=x;}}
