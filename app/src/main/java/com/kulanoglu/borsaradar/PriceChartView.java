...
public final class PriceChartView extends View {
    ...
    @Override protected void onDraw(Canvas canvas){super.onDraw(canvas);if(data==null||data.size()<2)return;if(visibleEnd<0||visibleEnd>=data.size())visibleEnd=data.size()-1;if(visibleCount<=0)visibleCount=data.size();visibleCount=Math.max(2,Math.min(visibleCount,data.size()));int start=Math.max(0,visibleEnd-visibleCount+1),count=visibleEnd-start+1;String symbol=data.get(visibleEnd).symbol,cur=currencySymbol(symbol);float left=getPaddingLeft()+dp(3),right=getWidth()-getPaddingRight()-dp(55),top=getPaddingTop()+dp(30),priceBottom=getHeight()-getPaddingBottom()-dp(105),volTop=priceBottom+dp(12),volBottom=getHeight()-getPaddingBottom()-dp(48);double min=Double.MAX_VALUE,max=-Double.MAX_VALUE,maxVol=0;for(int i=start;i<=visibleEnd;i++){MarketDataService.Candle c=data.get(i);min=Math.min(min,c.low);max=Math.max(max,c.high);maxVol=Math.max(maxVol,c.volume);}if(max<=min)max=min+1;double dataMin=min,dataMax=max;double rawSpan=dataMax-dataMin,padRange=Math.max(rawSpan*.06,Math.abs(dataMax)*.001);min=dataMin-padRange;max=dataMax+padRange;
        ...
        float slot=(right-left)/Math.max(1,count);drawVerticalGrid(canvas,left,right,top,priceBottom,start,count);drawRangeStats(canvas,left,top,start);float body=Math.max(dp(2),Math.min(dp(12),slot*.62f));for(int j=0;j<count;j++){MarketDataService.Candle c=data.get(start+j);float x=left+slot*(j+.5f);float yh=y(c.high,min,max,top,priceBottom),yl=y(c.low,min,max,top,priceBottom),yo=y(c.open,min,max,top,priceBottom),yc=y(c.close,min,max,top,priceBottom);int col=c.close>=c.open?Color.rgb(22,190,135):Color.rgb(235,65,85);paint.setColor(col);paint.setStrokeWidth(dp(1));canvas.drawLine(x,yh,x,yl,paint);paint.setStyle(Paint.Style.FILL);canvas.drawRect(x-body/2,Math.min(yo,yc),x+body/2,Math.max(Math.min(yo,yc)+dp(1),Math.max(yo,yc)),paint);if(showVolume&&maxVol>0){float vh=(float)(c.volume/maxVol)*(volBottom-volTop);paint.setColor(Color.argb(150,Color.red(col),Color.green(col),Color.blue(col)));canvas.drawRect(x-body/2,volBottom-vh,x+body/2,volBottom,paint);}}
        if(showChangeLine){double baseLine=data.get(start).close;float by=y(baseLine,min,max,top,priceBottom);paint.setColor(Color.rgb(110,130,150));paint.setStrokeWidth(dp(1));canvas.drawLine(left,by,right,by,paint);paint.setTextSize(dp(9));String bt="BAZ "+priceFmt(displayValue(baseLine,symbol));paint.setColor(Color.rgb(150,170,192));canvas.drawText(bt,right-paint.measureText(bt)-dp(4),Math.max(top+dp(10),by-dp(3)),paint);}if(showEma){drawEma(canvas,5,start,count,left,right,min,max,top,priceBottom,Color.rgb(255,193,7));drawEma(canvas,9,start,count,left,right,min,max,top,priceBottom,Color.rgb(77,208,225));drawEma(canvas,20,start,count,left,right,min,max,top,priceBottom,Color.rgb(186,104,200));drawEma(canvas,50,start,count,left,right,min,max,top,priceBottom,Color.rgb(255,138,101));}if(showBands)drawBollinger(canvas,20,start,count,left,right,min,max,top,priceBottom);if(showVwap)drawVwap(canvas,start,count,left,right,min,max,top,priceBottom);if(showLevels)drawSupportResistance(canvas,start,count,left,right,min,max,top,priceBottom);drawHighLowLabels(canvas,left,right,top,priceBottom,dataMin,dataMax,symbol,cur);
        ...
        if(showVolume)drawVolumeHeader(canvas,left,right,volTop,maxVol);drawTimeScale(canvas,left,right,volBottom,start,count);
    }
    @Override public boolean onTouchEvent(MotionEvent e){
        if(data==null||data.isEmpty())return false;
        scaleDetector.onTouchEvent(e);
        if(scaleDetector.isInProgress())return true;
        if(e.getAction()==MotionEvent.ACTION_DOWN){float lx=e.getX(),ly=e.getY();if(ly>=dp(28)&&ly<=dp(52)){float left=getPaddingLeft()+dp(3);if(lx>=left&&lx<legendEmaRight)showEma=!showEma;else if(lx<legendBbRight)showBands=!showBands;else if(lx<legendVwapRight)showVwap=!showVwap;else if(lx<legendLevelsRight)showLevels=!showLevels;else if(lx<legendVolumeRight)showVolume=!showVolume;else if(lx<legendChangeRight)showChangeLine=!showChangeLine;else return true;selectedIndex=-1;invalidate();return true;}downX=lastX=touchX=lx;panning=false;return true;}
    ...
    private void drawVolumeHeader(Canvas canvas,float left,float right,float volTop,double maxVol){paint.setStrokeWidth(dp(1));paint.setColor(Color.rgb(48,72,101));canvas.drawLine(left,volTop,right,volTop,paint);paint.setTextSize(dp(9));paint.setColor(Color.rgb(150,170,192));double lastVol=data.get(visibleEnd).volume;String t="HACİM  Son "+compact(lastVol)+"  •  Maks "+compact(maxVol);canvas.drawText(t,left,volTop+dp(10),paint);}
    ...
    private void drawLegend(Canvas canvas,float left,float y){paint.setTextSize(dp(9));String[] names={"EMA","BB","VWAP","S/R","HAC","BAZ"};boolean[] on={showEma,showBands,showVwap,showLevels,showVolume,showChangeLine};float x=left;for(int i=0;i<names.length;i++){paint.setColor(on[i]?Color.WHITE:Color.rgb(105,120,138));String t=(on[i]?"● ":"○ ")+names[i];canvas.drawText(t,x,y,paint);x+=paint.measureText(t)+dp(12);if(i==0)legendEmaRight=x;else if(i==1)legendBbRight=x;else if(i==2)legendVwapRight=x;else if(i==3)legendLevelsRight=x;else if(i==4)legendVolumeRight=x;else legendChangeRight=x;}}
