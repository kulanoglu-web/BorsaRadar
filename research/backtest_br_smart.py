# BR-SMART v14: regime + quality threshold selected only on development chunks
import json
...
def sig(s,i,day,mode):
    ...
    score=38*dhn+22*rvn+16*accn+12*adxq+8*extq+4*max(0,min(1,(bd+.02)/.10))
    return score,f

def sim(mode,minscore,days):
    ...
    for day in ds:
        ...
        if slots>0 and ok:
            ...
            for s,a in bi.data.items():
                ...
                z=sig(s,i,day,mode)
                if z and z[0]>=minscore:ca.append((z[0],s,i,z[1]))
...
configs=[(m,t) for m in ('BASE','STRONG','RECOVERY','DYNAMIC') for t in (0,45,55,65)]
devrows=[]
for mode,thr in configs:
    rr=[sim(mode,thr,ch) for ch in dev];active=[x for x in rr if x['n']];avg=sum(x['ret'] for x in rr)/len(rr);worst=min(x['ret'] for x in rr);dd=sum(x['dd'] for x in rr)/len(rr);pf=sum(min(5,x['pf']) for x in active)/len(active) if active else 0;n=sum(x['n'] for x in rr)
    # Require some activity; otherwise sparse configs can look artificially safe.
    score=(avg+.6*worst-.4*dd+.35*(pf-1)+min(.2,n/80))-(.35 if n<8 else 0)
    devrows.append({'mode':mode,'thr':thr,'avg':avg,'worst':worst,'dd':dd,'pf':pf,'n':n,'score':score})
best=max(devrows,key=lambda x:x['score']);hr=[sim(best['mode'],best['thr'],ch) for ch in hold];active=[x for x in hr if x['n']]
summary={'selected':best,'hold_avg':sum(x['ret'] for x in hr)/len(hr) if hr else 0,'hold_worst':min(x['ret'] for x in hr) if hr else 0,'hold_positive':sum(x['ret']>0 for x in hr),'hold_n':sum(x['n'] for x in hr),'hold_dd':sum(x['dd'] for x in hr)/len(hr) if hr else 0,'hold_pf':sum(min(5,x['pf']) for x in active)/len(active) if active else 0,'full':sim(best['mode'],best['thr'],DATES)}
lines=['# BR-SMART v14 Dev-Selected Quality Gate','Rejim ve yalnız dört kaba kalite eşiği ilk 6 dönemde seçildi; son 5 dönem tamamen dokunulmamış holdout. Eşikler 0/45/55/65; kapsamlı parameter mining yapılmadı. Terminal muhasebe düzeltmesi aktif. TUPRS/savunma hariç, maliyet dahil.','','## En iyi geliştirme adayları','|Mode|MinScore|Avg|Worst|DD|N|PF|','|---|---:|---:|---:|---:|---:|---:|']
for r in sorted(devrows,key=lambda x:x['score'],reverse=True)[:8]:lines.append(f'|{r["mode"]}|{r["thr"]}|{r["avg"]:.2f}%|{r["worst"]:.2f}%|{r["dd"]:.2f}%|{r["n"]}|{r["pf"]:.2f}|')
lines+=['',f'Seçilen: **{best["mode"]} / min skor {best["thr"]}**',f'Son 5 dokunulmamış dönem: ort **{summary["hold_avg"]:.2f}%**, en kötü **{summary["hold_worst"]:.2f}%**, pozitif **{summary["hold_positive"]}/{len(hr)}**, N **{summary["hold_n"]}**, DD **{summary["hold_dd"]:.2f}%**, PF **{summary["hold_pf"]:.2f}**. Tüm veri: **{summary["full"]["ret"]:.2f}%**, DD **{summary["full"]["dd"]:.2f}%**, PF **{summary["full"]["pf"]:.2f}**.','','|Holdout|Ret|DD|N|Win|PF|','|---:|---:|---:|---:|---:|---:|']
