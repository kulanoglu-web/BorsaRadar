# BR-SMART v11: fixed early-ignition model; no in-sample parameter selection
import json
...
def signal(s,i,day):
    ...
    # Seek ignition, not mature trend: acceleration gets most weight; high ADX/extension is penalized.
    accel=max(-.02,min(.08,f['acc']))
    dhn=max(0,min(1,dh/max(close*.006,.01)))
    rvn=max(0,min(1,(f['rv']-1.0)/1.1))
    accn=max(0,min(1,(accel+.005)/.05))
    adxq=max(0,1-abs(f['adx']-27)/15)
    extq=max(0,1-max(0,dist-.6)/1.4)
    score=38*dhn+22*rvn+16*accn+12*adxq+8*extq+4*max(0,min(1,(br-.40)/.20))
    return score,f,{'dh':dh,'dist':dist,'cp':cp}
...
def sim(days):
    ...
    for day in ds:
        ...
        for s in list(pos):
            ...
            a=bi.data[s];bar=a[i];p=pos[s];f=bi.feat(s,i);m=f['m'];p['hi']=max(p['hi'],bar['h']);p['age']+=1
            ...
            if bar['l']<=stop:ex=bar['o'] if bar['o']<stop else stop;reason='STOP'
                ...
                if p['age']>=4 and un<-.007 and gain<.02:ex=bar['o'];reason='FAIL'
        ...
        if slots>0 and br>=.42:
            ...
            for sc,s,i,f,x in ca[:slots]:
                px=bi.data[s][i]['o'];atr=max(f['m']['atr'],px*.01);hard=px-max(1.15*atr,px*.012)
                q=int(min(cash/(max(1,slots)*px*(1+FEE)),START*.0022/max(.01,px-hard)))
                if q<=0:continue
    ...
    return {'ret':(cash/START-1)*100,'dd':dd*100,'n':len(tr),'win':100*len(wins)/len(rets) if rets else 0,'pf':gp/gl if gl else (99 if gp else 0),'avgw':sum(wins)/max(1,len(wins)),'avgl':sum(loss)/max(1,len(loss)),'stop':sum(x[2]=='STOP' for x in tr)}

dates=bi.dates
chunks=[dates[i:i+22] for i in range(0,len(dates)-21,22)]
rows=[sim(ch) for ch in chunks]
...
lines=['# BR-SMART v11 Fixed Ignition','Parametre seçimi yok. Sabit erken-ateşleme modeli: hacim + MACD histogram hızlanması + sınırlı ADX + EMA20ye yakınlık + piyasa genişliği. TUPRS/savunma hariç; maliyet dahil; her 22 işlem günü ayrı test.','',f'Ort dönem **{summary["avg"]:.2f}%**, en kötü **{summary["worst"]:.2f}%**, pozitif **{summary["positive"]}/{summary["chunks"]}**, toplam işlem **{summary["trades"]}**, ort DD **{summary["avgdd"]:.2f}%**, ort PF **{summary["avgpf"]:.2f}**. Tüm veri: **{summary["full"]["ret"]:.2f}%**, DD **{summary["full"]["dd"]:.2f}%**, PF **{summary["full"]["pf"]:.2f}**.','','|Dönem|Ret|DD|N|Win|PF|AvgW|AvgL|Stop|','|---:|---:|---:|---:|---:|---:|---:|---:|---:|']
for i,r in enumerate(rows,1):lines.append(f'|{i}|{r["ret"]:.2f}%|{r["dd"]:.2f}%|{r["n"]}|{r["win"]:.1f}%|{r["pf"]:.2f}|{r["avgw"]:.2f}%|{r["avgl"]:.2f}%|{r["stop"]}|')
