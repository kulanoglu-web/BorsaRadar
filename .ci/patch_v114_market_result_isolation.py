...
if 'radarResultMarket' not in s:
    s=s.replace(field,field+'\n    private int radarResultMarket=-1;\n    private final java.util.HashMap<Integer,java.util.ArrayList<RadarItem>> radarResultsByMarket=new java.util.HashMap<>();',1)
marker='    private String radarMarketLabel()'
helper='''    private void saveRadarMarketSnapshot(int market){
        try{if(radarResults!=null)radarResultsByMarket.put(market,new java.util.ArrayList<RadarItem>(radarResults));}catch(Throwable ignored){}
    }
    private void restoreRadarMarketSnapshot(int market){
        try{radarResults.clear();java.util.ArrayList<RadarItem> a=radarResultsByMarket.get(market);if(a!=null)radarResults.addAll(a);radarResultMarket=market;}catch(Throwable ignored){}
    }
