...
helpers='''    private String portfolioBackupJson(){
        try{
            ...
            JSONObject profiles=new JSONObject(); for(int m=0;m<3;m++) profiles.put(String.valueOf(m),new JSONArray(sp.getString("portfolio_"+m,"[]")));
    ...
    private void restorePortfolioBackup(android.net.Uri uri,boolean replace){try{String json;try(java.io.InputStream in=getContentResolver().openInputStream(uri)){java.util.Scanner sc=new java.util.Scanner(in,"UTF-8").useDelimiter("\\A");json=sc.hasNext()?sc.next():"";}JSONObject root=new JSONObject(json);if(!"BorsaRadarPortfolioBackup".equals(root.optString("format")))throw new Exception("format");android.content.SharedPreferences sp=getSharedPreferences(PREFS,MODE_PRIVATE);android.content.SharedPreferences.Editor pe=sp.edit();JSONObject profiles=root.optJSONObject("realProfiles");if(profiles!=null){for(int m=0;m<3;m++){String key="portfolio_"+m;JSONArray incoming=profiles.optJSONArray(String.valueOf(m));if(incoming==null)continue;if(replace)pe.putString(key,incoming.toString());else{JSONArray merged=new JSONArray(sp.getString(key,"[]"));java.util.HashSet<String> seen=new java.util.HashSet<>();for(int j=0;j<merged.length();j++){JSONObject x=merged.optJSONObject(j);if(x!=null)seen.add(x.optString("s"));}for(int j=0;j<incoming.length();j++){JSONObject o=incoming.optJSONObject(j);if(o!=null&&!seen.contains(o.optString("s"))){merged.put(o);seen.add(o.optString("s"));}}pe.putString(key,merged.toString());}}}else{JSONArray old=root.optJSONArray("real");if(old!=null){String key="portfolio_"+primaryMarket();JSONArray converted=replace?new JSONArray():new JSONArray(sp.getString(key,"[]"));java.util.HashSet<String> seen=new java.util.HashSet<>();for(int j=0;j<converted.length();j++){JSONObject x=converted.optJSONObject(j);if(x!=null)seen.add(x.optString("s"));}for(int j=0;j<old.length();j++){JSONObject o=old.getJSONObject(j);String sy=o.getString("symbol");if(!seen.contains(sy)){JSONObject n=new JSONObject();n.put("s",sy);n.put("q",o.getInt("qty"));n.put("c",o.getDouble("cost"));converted.put(n);seen.add(sy);}}pe.putString(key,converted.toString());}}pe.apply();android.content.SharedPreferences.Editor fe=getSharedPreferences("paper_portfolio",MODE_PRIVATE).edit();if(replace)fe.clear();JSONObject fake=root.optJSONObject("fake");if(fake!=null){java.util.Iterator<String> it=fake.keys();while(it.hasNext()){String k=it.next();fe.putString(k,fake.optString(k));}}fe.apply();loadPortfolio();Toast.makeText(this,"Gerçek + Fake portföy geri yüklendi",Toast.LENGTH_LONG).show();showPortfolio();}catch(Exception e){Toast.makeText(this,"Yedek okunamadı veya uyumsuz",Toast.LENGTH_LONG).show();}}
    private void handlePortfolioFileResult(int requestCode,int resultCode,Intent data){if(resultCode!=RESULT_OK||data==null||data.getData()==null)return;final android.net.Uri u=data.getData();if(requestCode==990&&pendingPortfolioBackupJson!=null){try(java.io.OutputStream out=getContentResolver().openOutputStream(u)){out.write(pendingPortfolioBackupJson.getBytes(java.nio.charset.StandardCharsets.UTF_8));Toast.makeText(this,"Portföy yedeği kaydedildi",Toast.LENGTH_LONG).show();}catch(Exception e){Toast.makeText(this,"Yedek kaydedilemedi",Toast.LENGTH_LONG).show();}finally{pendingPortfolioBackupJson=null;}}else if(requestCode==991){new AlertDialog.Builder(this).setTitle("Portföy yedeğini geri yükle").setMessage("Birleştir mevcut kayıtları korur. Yerine koy üç Gerçek portföyü ve Fake portföyü yedekteki kayıtlarla değiştirir.").setPositiveButton("Birleştir",(d,w)->restorePortfolioBackup(u,false)).setNegativeButton("Yerine koy",(d,w)->restorePortfolioBackup(u,true)).setNeutralButton("İptal",null).show();}}
...
# Find showPortfolio method structurally, then insert after its first shell(...) call regardless of localized title.
start=s.find('    private void showPortfolio()')
if start<0: raise SystemExit('showPortfolio method missing')
shellpos=s.find('shell(',start)
semi=s.find(';',shellpos)
...
# Intent may not be imported in the original source.
...
# Integrate with existing activity result callback or add one.
