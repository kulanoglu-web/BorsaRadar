...
# Phone runtime uses only recent background data; long history remains a research/backtest concern, not UI load.
...
s=s.replace('"Mantık: güçlü AL/ERKEN sinyaliyle giriş; ATR + EMA50 tabanlı ilk stop; ATR trailing ve trend/MACD bozulmasında çıkış."','"Metodlar: BR-Pulse (trend verimi + ATR momentum + hacim baskısı), FlowBreak (para akışı + göreli hacim + kırılım), TrendGuard (trend + MACD + ADX + RSI)."')

# Replace decision method so every recommendation is driven by the ensemble percentage.
...
# Replace decisionColor body to follow ensemble labels.
...
# Add custom-method summary to explanation and consensus.
s=s.replace('return "Neye göre: " + android.text.TextUtils.join(" • ", why) + ".";',
'''MethodEngine.Result m = MethodEngine.analyze(s);
        return "Neye göre: " + android.text.TextUtils.join(" • ", why) + ". • " + m.summary
                + " • Bu yüzde kazanç garantisi değil, indikatör/metod uyum gücüdür.";''')
s=s.replace('return "İndikatör uzlaşması: " + buy + " AL • " + neutral + " NÖTR • " + sell + " SAT";',
'''MethodEngine.Result m = MethodEngine.analyze(s);
        return "Analiz %" + String.format(Locale.US, "%.0f", m.percent)
                + " • " + buy + " AL • " + neutral + " NÖTR • " + sell + " SAT";''')

# Radar cards: percentage first, no one-year backtest emphasis.
...
# Strategy thresholds use ensemble percentage rather than raw score alone.
...
# Graph label: explicit timeframe.
...
g=re.sub(r"versionName\s+'[^']+'", "versionName '3.1.0'", g)
