...
if 'String normalized=MarketDataService.normalizeSymbol(symbol);' not in s:
    old='''  List<MarketDataService.Candle>d=MarketDataService.fetchSeries(symbol,range,interval,0);'''
    new='''  String normalized=MarketDataService.normalizeSymbol(symbol);
  List<MarketDataService.Candle>d;
  try{d=MarketDataService.fetchSeries(symbol,range,interval,0);}
   ...
   if(normalized.endsWith(".IS")&&!"1d".equals(interval)){
    ...
    d=MarketDataService.fetchSeries("BIST:"+normalized.substring(0,normalized.length()-3),fallback,interval,0);
   }else throw first;
  }'''
