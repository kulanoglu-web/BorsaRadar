...
# Hourly radar: never leave a stale running flag blocking a manual refresh.
...
new='''private void scanShortTerm(String mode){
        // A manual hourly refresh must always be able to start; stale flags from interrupted scans are discarded.
        if(shortScanRunning && shortScanDone.get()>0 && shortScanDone.get()<marketSymbols(primaryMarket()).length){
...
# Make hourly refresh button usable even while a previous scan is stale/running.
...
# Add a dedicated Fake Portfolio navigation button next to real Portfolio.
nav='''        Button p=button('''
i=s.find(nav)
if i>=0 and 'Button fake=button("Fake"' not in s[max(0,i-500):i+800]:
    e=s.find('\n',i)
...
# Dedicated Fake Portfolio screen, legacy-safe and clearly separate from real holdings.
...
if 'private void showFakePortfolio()' not in s:
    method=r'''private void showFakePortfolio(){
        ...
        for(java.util.Map.Entry<String,?> en:all.entrySet()){
            try{
                String symbol=en.getKey();String raw=String.valueOf(en.getValue());String[] z=raw.split("\\|");
                ...
                double now=buy; RadarItem ri=findRadarItem(symbol); if(ri!=null&&ri.r!=null&&ri.r.price>0)now=ri.r.price;
                ...
                c.addView(txt("Adet "+qty+" • Alış "+money(symbol,buy)+" • Güncel "+money(symbol,now),14,Color.LTGRAY));
                c.addView(txt("K/Z "+(pnl>=0?"+":"")+String.format(java.util.Locale.US,"%.2f",pnl)+" • Değer "+String.format(java.util.Locale.US,"%.2f",val),14,pnl>=0?GREEN:RED));
                c.setOnClickListener(v->openStock(symbol));content.addView(c);spacer(6);
            }catch(Exception ignored){}
        }
        if(valid>0){double pnl=totalNow-totalCost,pct=totalCost>0?100*pnl/totalCost:0;LinearLayout sum=card();sum.addView(bold("TOPLAM FAKE PERFORMANS",16,Color.WHITE));sum.addView(bold((pnl>=0?"▲ ":"▼ ")+String.format(java.util.Locale.US,"%+.2f%% • %.2f",pct,pnl),20,pnl>=0?GREEN:RED));content.addView(sum,0);}
    }
