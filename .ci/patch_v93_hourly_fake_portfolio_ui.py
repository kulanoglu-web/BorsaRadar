...
if i>=0 and 'Button fake=button("Fake"' not in s[max(0,i-500):i+900]:
    e=s.find('\n',i)
    s=s[:e+1]+'        Button fake=button("Fake",AMBER);\n'+s[e+1:]
...
if 'private void showFakePortfolio()' not in s:
    method=r'''private void showFakePortfolio(){
        ...
        head.addView(bold("🧪 FAKE PORTFÖY",21,AMBER));
        ...
        for(java.util.Map.Entry<String,?> en:all.entrySet()){
            try{
                ...
                double now=buy;
                synchronized(radarResults){for(RadarItem item:radarResults){if(item.symbol.equals(symbol)&&Double.isFinite(item.price)&&item.price>0){now=item.price;break;}}}
                if(now==buy){synchronized(shortRadarResults){for(RadarItem item:shortRadarResults){if(item.symbol.equals(symbol)&&Double.isFinite(item.price)&&item.price>0){now=item.price;break;}}}}
        ...
        if(valid>0){double pnl=totalNow-totalCost,pct=totalCost>0?100*pnl/totalCost:0;LinearLayout sum=card();sum.addView(bold("TOPLAM FAKE PERFORMANS",16,Color.DKGRAY));sum.addView(bold((pnl>=0?"▲ ":"▼ ")+String.format(java.util.Locale.US,"%+.2f%% • ",pct)+String.format(java.util.Locale.US,"%.2f",pnl),20,pnl>=0?GREEN:RED));content.addView(sum,0);}
    }
