...
detail_new='''        shell("Hisse Detayı");
        ...
        LinearLayout tabs=new LinearLayout(this);tabs.setOrientation(LinearLayout.HORIZONTAL);String[] tabNames={"Genel","Grafik","Haber","KAP","Finansal"};for(String t:tabNames){final String selected=t;Button b=button(t,t.equals("Genel")?Color.rgb(25,105,210):NAVY2);tabs.addView(b,new LinearLayout.LayoutParams(0,-2,1));b.setOnClickListener(v->{if("Genel".equals(selected)||"Grafik".equals(selected)){analyzeStock(symbol);}else{Toast.makeText(this,selected+" verileri yükleniyor",Toast.LENGTH_SHORT).show();analyzeStock(symbol);}});}content.addView(tabs);spacer(8);
...
if portfolio_anchor in s and 'private void showMore()' not in s:
    more='''    private void showMore() {
        ...
        content.addView(bold("BorsaRadar V3.0.2",16,Color.WHITE));content.addView(txt("V18 • build 32",12,Color.LTGRAY));spacer(10);for(String m:new String[]{"Piyasa Takvimi","Sektörler","Favorilerim","Alarmlar","Hisse Karşılaştırma","Döviz / Altın / Emtia","Ekonomik Veriler","Ayarlar","Yardım & Destek","Hakkında","Çıkış Yap"}){Button b=button(m,NAVY2);b.setGravity(Gravity.LEFT|Gravity.CENTER_VERTICAL);content.addView(b,new LinearLayout.LayoutParams(-1,dp(50)));}
    }
